...
rawhdr.fileinfo.actual_filename = [ lay_file_name(1:end-4) '.dat' ];
...
header.nChan       = str2double(rawhdr.fileinfo.waveformcount);
header.calibration = str2double(rawhdr.fileinfo.calibration);
...
dat_file_ID = fopen(rawhdr.fileinfo.actual_filename);
fseek( dat_file_ID, 0, 'eof' );
header.datapoints = ftell( dat_file_ID ) / nBytes / header.nChan;
fclose( dat_file_ID );

%% skip reading the comments if requested
if( nargin == 2 && ~readComments )
